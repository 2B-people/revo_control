#include "../include/revoMotor/revodriver.h"
#include "../../../devel/.private/revoMotor/include/revoMotor/motorinfo.h"
#include "../../../devel/.private/revoMotor/include/revoMotor/motorcmd.h"
#include "../../../devel/.private/revoMotor/include/revoMotor/statecmd.h"
...
{
    // 创建ros节点jason
    ros::init(argc, argv, "JASON");
    ros::NodeHandle nh;
    // 创建三个发布者
    ros::Publisher motorinfo0 = nh.advertise<revoMotor::motorinfo>("Motorinfo0", 100);
    ...
    ros::Subscriber statercmd = nh.subscribe("statecmd", 1000, statecmdcallback);
    ros::Subscriber motorcmd = nh.subscribe("Motorcmd", 1000, c2dcallback);
    ...
    while (ros::ok())
    {
        ...
        if (n != 0)
        {
            ...
            for (size_t i = 0; i < 1; i++)
            {
                ...
                motorinfoset[i].position = uint_to_float(temp, parm_.P_MIN, parm_.P_MAX, 16);
                temp = ((buffer[3 + 8 * i] << 4) | ((buffer[4 + 8 * i] & 0x00ff) >> 4));
                motorinfoset[i].velocity = uint_to_float(temp, parm_.V_MIN, parm_.V_MAX, 12);
                temp = (((buffer[4 + 8 * i] & 0x0f) << 4) | (0x00ff & buffer[5 + 8 * i]));
                motorinfoset[i].torque = uint_to_float(temp, parm_.T_MIN, parm_.T_MAX, 12);
